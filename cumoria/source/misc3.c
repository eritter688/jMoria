...
#ifdef COLOUR_GFX
  setattr(Colour_Sub_Title);
  put_buffer(stat_names[stat], 6+stat, STAT_COLUMN);
  if(py.stats.cur_stat[stat] <= py.stats.max_stat[stat] / 5)
    setattr(Colour_Warning);
  else
    if(py.stats.cur_stat[stat] < py.stats.max_stat[stat])
...
{
#ifdef COLOUR_GFX
  if(py.misc.mana && py.misc.cmana <= py.misc.mana / 5)
    setattr(Colour_Warning);
  else
...
#ifdef COLOUR_GFX
  setattr(Colour_Sub_Title);
  prt("LEV :", 13, STAT_COLUMN);
  prt("EXP :", 14, STAT_COLUMN);
  prt("MANA:", 15, STAT_COLUMN);
  prt("MHP :", 16, STAT_COLUMN);
  prt("CHP :", 17, STAT_COLUMN);
  prt("AC  :", 19, STAT_COLUMN);
  prt("GOLD:", 20, STAT_COLUMN);
  prt_level();
  prt_experience();
  prt_cmana();
  prt_mhp();
...
void put_stats()
{
  ...
  for (i = 0; i < 6; i++)
    {
      ...
      if (py.stats.max_stat[i] / 5 >= py.stats.cur_stat[i])
        {
          setattr(Colour_Warning);
  ...
  setattr(Colour_Sub_Title);
  put_buffer("+ To Hit    : ",  9, 1);
  put_buffer("+ To Damage : ", 10, 1);
  put_buffer("+ To AC     : ", 11, 1);
  put_buffer("  Total AC  : ", 12, 1);
  ...
  if(m_ptr->dis_tac < 0)
    setattr(Colour_Warning);
  prt_int(m_ptr->dis_tac, 11, 15);

  setattr(Colour_Title);
  prt_int(m_ptr->dis_ac, 12, 15);
#else
  m_ptr = &py.misc;
  for (i = 0; i < 6; i++)
    {
...
#ifdef COLOUR_GFX
/* Returns a colour of x depending on y                 -EMP-   */
char likert_colour(x, y)
int x, y;
{
  switch((x/y))
    {
      case -3: case -2: case -1: case 0: case  1: return(Colour_Warning);         
      case  2: case  3: case  4: case 5:          return(Colour_Attention);
      case  6: case  7: case  8: default:         return(Colour_OK);
    }
}
#endif
...
void put_misc1()
{
  ...
  setattr(Colour_Sub_Title);
  put_buffer("Age          : ", 2, 38);
  put_buffer("Height       : ", 3, 38);
  put_buffer("Weight       : ", 4, 38);
  put_buffer("Social Class : ", 5, 38);
  setattr(Colour_Field);
  prt_int((int)m_ptr->age, 2, 53);
  prt_int((int)m_ptr->ht,  3, 53);
  prt_int((int)m_ptr->wt,  4, 53);
  prt_int((int)m_ptr->sc,  5, 53);
  setattr(Colour_Default);
#else
  m_ptr = &py.misc;
...
void put_misc2()
{
  ...
  setattr(Colour_Sub_Title);
  put_buffer("Level      : ",  9, 29);
  put_buffer("Experience : ", 10, 29);
  put_buffer("Max Exp    : ", 11, 29);
  put_buffer("Exp to Adv.: ", 12, 29);
  put_buffer("Gold       : ", 13, 29);

  put_buffer("Max Hit Points : ",  9, 52);
  put_buffer("Cur Hit Points : ", 10, 52);
  put_buffer("Max Mana       : ", 11, 52);
  put_buffer("Cur Mana       : ", 12, 52);

  setattr(Colour_Field);
  prt_int((int)m_ptr->lev, 9, 42);

  if(m_ptr->max_exp && m_ptr->exp <= m_ptr->max_exp / 5)
    setattr(Colour_Warning);
  else
    if(m_ptr->exp < m_ptr->max_exp)
      setattr(Colour_Attention);
    else
      setattr(Colour_OK);
  prt_long(m_ptr->exp, 10, 42);

  setattr(Colour_Field);
  prt_long(m_ptr->max_exp, 11, 42);

  if (m_ptr->lev == MAX_PLAYER_LEVEL)
    {
    setattr(Colour_Non_Applicable);
    put_buffer("******", 12, 42);
    }
  else
    {
    setattr(Colour_Field);
    prt_long((int32)(player_exp[m_ptr->lev-1] * m_ptr->expfact / 100), 12, 42);
    }

  setattr(Colour_Field);
  prt_long(m_ptr->au, 13, 42);

  prt_int(m_ptr->mhp, 9, 69);

  if(m_ptr->chp <= m_ptr->mhp / 5)
    setattr(Colour_Warning);
  else
    if(m_ptr->chp < m_ptr->mhp)
      ...
      setattr(Colour_OK);
  prt_int(m_ptr->chp, 10, 69);

  setattr(Colour_Field);
  prt_int(m_ptr->mana, 11, 69);
...
void put_misc3()
{
  ...
  setattr(likert_colour(xbth, 12));
  put_buffer (likert (xbth, 12), 16, 15);
  setattr(likert_colour(xbthb, 12));
  put_buffer (likert (xbthb, 12), 17, 15);
  setattr(likert_colour(xsave, 6));
  put_buffer (likert (xsave, 6), 18, 15);

  setattr(likert_colour(xstl, 1));
  put_buffer (likert (xstl, 1), 16, 42);
  setattr(likert_colour(xdis, 8));
  put_buffer (likert (xdis, 8), 17, 42);
  setattr(likert_colour(xdev, 6));
  put_buffer (likert (xdev, 6), 18, 42);

  setattr(likert_colour(xfos, 3));
  put_buffer (likert (xfos, 3), 16, 69);
  setattr(likert_colour(xsrh, 6));
...
#if defined(MAC) || defined(AMIGA) || defined(ARC_RISCOS2)
...
static void gain_level()
{
  ...
  msg_print(out_val);
  msg_print(0L); /* Force a -More- prompt to highlight such an important event*/
  calc_hitpoints();
...
#ifdef COLOUR_GFX
  if(p_ptr->max_exp && p_ptr->exp <= p_ptr->max_exp / 5)
    setattr(Colour_Warning);
  else
    if(p_ptr->exp < p_ptr->max_exp)
      setattr(Colour_Attention);
