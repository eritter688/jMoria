...
/* The Archimedes version can have floating windows to put the recall */
/* info in, and this is done automatically when you look or attack or */
/* get hit. In these situations you dont want to pause for the player */
/* but when scanning a list of monsters via the '/' command we have   */
/* to pause on each entry. The pauseflag covers for this desiscion.   */

/*  NEW: Now forthe Macintosh!  - RJK -*/

#if defined(ARC_RISCOS2) || defined(MAC)
int roff_recall(mon_num, pauseflag)
int mon_num;
int pauseflag;
#else
...
#endif
{
  ...
  extern void recallwindow_clear(void); /* FN prototype */
#endif
  if (recallwindowopen)
    {
      if (recallwindowlast == mon_num && recalllastkilled == c_recall[mon_num].r_kills)
        return 0;  /* Don't re-draw same info */
      recallwindowlast = mon_num;
      recalllastkilled = c_recall[mon_num].r_kills;
      recallwindow_clear();
    }
  else
    {
...
#if defined(ARC_RISCOS2) || defined(MAC)
  if (!recallwindowopen)  /* Don't print message if writing to recall window */
    {
#ifdef COLOUR_GFX
  ...
  if (wizard)
    *mp = save_mem;
#if defined(ARC_RISCOS2) || defined(MAC)
  if (recallwindowopen && !pauseflag)  /* Don't pause if writing to recall window */
    return(0);
#endif
  return inkey();
}
...
#ifdef ARC_RISCOS2
  extern int recallwindowopen;
  extern int recalllinelength;
  int linelength;

  extern void recallwindow_prt(vtype, int, int);  /* FN prototype */
#endif
  ...
  while (*p)
    {
      ...
      *roffp = *p;
#if defined(ARC_RISCOS2)
      if(recallwindowopen)
        linelength = recalllinelength;
      else
        linelength = sizeof(roffbuf)-1;
      if (*p == '\n' || roffp >= roffbuf + linelength)
#else
...
#ifdef ARC_RISCOS2
          if (recallwindowopen)
            recallwindow_prt(roffbuf, roffpline, 0);
          else
