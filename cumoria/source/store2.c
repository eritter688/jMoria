...
#ifdef COLOUR_GFX /* All colour code added by JCW (execpt those added by EMP)*/
...
*/
extern char itemcolour(register inven_type *t_ptr, int knowall)
{
  register int type;

  /* Is it of a magical type of item? */
  type = t_ptr->tval;

  if (t_ptr->ident & ID_DAMD) /* before anything else, check if its damned */
    return (Colour_Damned);

  if (t_ptr->ident & ID_MAGIK) /* return green if its known magik */
    return (Colour_Magik);

  if (t_ptr->ident & ID_EMPTY) /* return mid grey if its known empty */
    return (Colour_Empty);            /* (not sure about this one)          */

 if (type == TV_AMULET)
    return(Colour_Inventory_Amulet);

  if (type == TV_RING)
    return(Colour_Inventory_Ring);

  if (type == TV_WAND)
   return(Colour_Inventory_Wand);

  if (type == TV_STAFF)
    return(Colour_Inventory_Staff);

  if (type == TV_POTION1 || type == TV_POTION2)
    return(Colour_Inventory_Potion);

  /* Is it a magical book/scroll - return parchment colour */
  if (type == TV_SCROLL1    || type == TV_SCROLL2)
    return(Colour_Inventory_Scroll);

  if (type == TV_MAGIC_BOOK || type == TV_PRAYER_BOOK)
    return(Colour_Inventory_Book);

  if (!knowall)   /* Do we check if player knows about the enchantment? */
    if ((!(t_ptr->ident & ID_KNOWN2)))
      return(Colour_Inventory_Misc);
  ...
  */
  if (t_ptr->tohit > 0 || t_ptr->todam > 0 || t_ptr->toac > 0)
    return(Colour_Inventory_Enchanted);
  ...
  while (start < stop)
    {
...
#ifdef COLOUR_GFX
      setattr(Colour_Title);                             /* Letter a) b) etc */
      (void) sprintf(out_val2, "%c) ", 'a'+i);
      prt(out_val2, i+5, 0);
      setattr(itemcolour(i_ptr, TRUE));       /* Name coloured from itemtype */
      (void) sprintf(out_val2, "%s", out_val1);
      prt(out_val2, i+5, 3);
#else
      ...
      if (x <= 0)
        {
          ...
          if (value <= 0)
            value = 1;
#ifdef COLOUR_GFX
          setattr(Colour_Gold);                       /* JCW Unfixed prices */
#endif
      ...
      {
#ifdef COLOUR_GFX
        setattr(Colour_OK);                             /* JCW Fixed prices */
#endif
        (void) sprintf(out_val2,"%9ld [Fixed]", x);
      }

      prt(out_val2, i+5, 59);     /* JCW if colour use same as desc. (above) */
      i++;
      start++;
    }
  if (i < 12)
...
int store_num, pos;
{
...
#ifdef COLOUR_GFX
  {
    ...
    setattr(itemcolour(i_ptr, TRUE)); /* JCW: pretty colours */
  }
#endif
  ...
  {
#ifdef COLOUR_GFX
    setattr(Colour_OK)                      ;               /* Fixed prices */
#endif
...
int num_offer;
{
  ...
  do
    {
#ifdef COLOUR_GFX
      setattr(Colour_Plain_Text); /* Same style as msg_prt */
#endif
      prt(comment, 0, 0);
