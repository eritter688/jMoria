...
#ifdef COLOUR_GFX
#ifndef ARC_RISCOS2
#include "colour.h"     /*include colour anyway */
#else
...
#if defined(MSDOS) || defined(VMS) || defined(MAC) || defined(ARC_RISCOS2)
...
/*  Use seperate routine for mac using TEXT resources -RJK- */
...
char *filename;
    ...
    bigvtype    tmp_str;
    Handle      HelpResource;
    char        *helpend, *helpstart;
    short       ResourceSize;
    short       pos, i;
    char        input;
    
    strcpy(tmp_str, filename);
    HelpResource = GetNamedResource('TEXT', CtoPstr(tmp_str));
    if (HelpResource == 0L)
        {
        (void) sprintf(tmp_str, "Could not find help resource \"%s\".", filename);
        prt (tmp_str, 0, 0);
        return;
        }       
    
    HLock(HelpResource);
    ResourceSize = GetHandleSize(HelpResource);
    helpend = *HelpResource;
    save_screen();
  
    pos = 0;
    
    while (pos < ResourceSize)
        {
        clear_screen();
#ifdef COLOUR_GFX
      ...
      for (i = 0; i < 23; i++)
            {
            helpstart = helpend;
            while ((*helpend != '\r') && (pos++ < ResourceSize))
                helpend++;
            *helpend++ = 0;
            put_buffer (helpstart, i, 0);
            }
#ifdef COLOUR_GFX
    ...
    HUnlock(HelpResource);
    ReleaseResource(HelpResource);
    }
#else
{
  bigvtype tmp_str;
...
#ifdef ARC_RISCOS2
  /* The C compiler doesn't come with any low level io libraries, thus everything
     has to use fopen, fseek, etc */
