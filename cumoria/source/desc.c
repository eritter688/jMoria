  ...
  for (i = 3; i < MAX_COLORS; i++)
    {
      ...
      colors[j] = tmp;
#ifdef COLOUR_GFX
      tmp_col = potion_colour[i];
      potion_colour[i] = potion_colour[j];
      potion_colour[j] = tmp_col;
#endif
    }
  for (i = 0; i < MAX_WOODS; i++)
    {
      ...
      woods[j] = tmp;
#ifdef COLOUR_GFX
      tmp_col = wood_colour[i];
      wood_colour[i] = wood_colour[j];
      wood_colour[j] = tmp_col;
#endif
    }
  for (i = 0; i < MAX_METALS; i++)
    {
      ...
      metals[j] = tmp;
#ifdef COLOUR_GFX
      tmp_col = metal_colour[i];
      metal_colour[i] = metal_colour[j];
      metal_colour[j] = tmp_col;
#endif
    }
  for (i = 0; i < MAX_ROCKS; i++)
    {
      ...
      rocks[j] = tmp;
#ifdef COLOUR_GFX
      tmp_col = rock_colour[i];
      rock_colour[i] = rock_colour[j];
      rock_colour[j] = tmp_col;
#endif
     }
  for (i = 0; i < MAX_AMULETS; i++)
    {
      ...
      amulets[j] = tmp;
#ifdef COLOUR_GFX
      tmp_col = amulet_colour[i];
      amulet_colour[i] = amulet_colour[j];
      amulet_colour[j]  = tmp_col;
#endif
    }
  for (i = 0; i < MAX_MUSH; i++)
    {
      ...
      mushrooms[j] = tmp;
#ifdef COLOUR_GFX
      tmp_col = mushroom_colour[i];
      mushroom_colour[i] = mushroom_colour[j];
      mushroom_colour[j] = tmp_col;
#endif
    }
