...
#ifdef ARC_RISCOS2 /* to stop a nasty warning that upsets makefiles */
#ifdef DEBUGDEF
static FILE *logfile;
#endif
#else
...
#else
/* remove() = failed = -1 ??? */
      testfile = fopen(savefile,"r");
      if(testfile != 0)
        {
          fclose(testfile);
          if(get_check("File exists. Delete old savefile?") == TRUE)
            {
              if(remove(savefile) == -1)
                {
                  (void) sprintf(temp, "Can't delete '%s'", savefile);
                  msg_print(temp);
                  prt("New Savefile [ESC to give up]:", 0, 0);
...
#else
  /* No low-level io calls */
  /* The logic behind the above low-level code could be */
  /* made somewhat clearer IMO...                       */
  if((tempfname = fopen(fnam, "wb")) == 0)
    {
      fd = -1;
      if(from_savefile || (wizard && get_check("Can't make new savefile. Overwrite old?")))
        {
          if((tempfname = fopen(fnam, "wb")) == 0)
              fd = -1;
          else
...
#if defined(atarist) || defined(ATARI_ST) || defined(THINK_C) || defined(MSDOS) || defined(ARC_RISCOS2)
...
#else
  if((tempfname = fopen(savefile, "rb")) == 0)
    {
      signals();
      msg_print("Savefile does not exist (or cannot be opened.)");
      return FALSE;     /* Don't bother with messages here. File not accessable. */
    }
  else
      fclose(tempfname);
#endif
...
#else
#if defined(ATARI_ST) || defined (ARC_RISCOS2)
  else if (FALSE)
#else
