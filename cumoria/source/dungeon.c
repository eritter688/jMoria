...
#ifdef ARC_RISCOS2
/* the defination for the disturb? key call        */
/* the int's should be BOOL's but it'll do for now... */
extern int screen_getkey(char *, int);
#endif

#ifdef MAC
extern int GameCanAcceptCommands;
extern int Quitting;
#endif
...
#ifdef ARC_RISCOS2
      /* On the Arc checking for input is also expensive (see Mac below) */
      /* so we do it only every 16 turns.                                */
      if (!(turn & 0xF) && ((command_count > 0) || find_flag || (f_ptr->rest != 0)))
        if (screen_getkey(&dummyinput, TRUE))
          disturb(0, 0);
#else
#ifdef MAC
              ...
              else
                {
                  ...
                  if (command_count > 0)
                  ...
                  else
                    {
                      msg_flag = FALSE;
#ifdef MAC
                      GameCanAcceptCommands = TRUE;
#endif
                      command = inkey();
#ifdef MAC
                      GameCanAcceptCommands = FALSE;
#endif
                      i = 0;
          ...
          if (!savefileset)
            ...
            switch( confirm_save(TRUE) )
                ...
                case saveNo:
                    msg_print("Not Saved.");
                    put_qio();
                    (void) strcpy (died_from, "(Not Saved)");
                    noscore = TRUE;
                    /* To fool HighScore into not adding
                    this character to the highscore list. I proberly
                    Shouldn't do this */
    ...
    if (t_list[c_ptr->tptr].tval == TV_UP_STAIR)
      {
        ...
        put_qio();  /* RJK - force Msg display before generate */
