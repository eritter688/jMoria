  ...
  do
    {
      ...
      (void) sprintf(tmp_str, "%c) ", k+'a');
      put_buffer(tmp_str, m, l);
      setattr(Colour_Input);
      (void) sprintf(tmp_str, "%s", race[j].trace);
      put_buffer(tmp_str, m, l+3);
...
static void get_sex()
{
  ...
  put_buffer("f)", 21, 16);
  setattr(Colour_Input);
  put_buffer("Male", 21, 5);
  put_buffer("Female", 21, 19);
  setattr(Colour_Title);
#else
  put_buffer("Choose a sex (? for Help):", 20, 2);
      ...
      if (race[i].rtclass & mask)
        {
          ...
          put_buffer(tmp_str, m, l);
          setattr(Colour_Input);
          (void) sprintf(tmp_str, "%s", class[j].title);
          put_buffer(tmp_str, m, l+3);
#else
