...
#if defined(ARC_RISCOS2) || defined(MAC)
#ifdef ARC_RISCOS2
      (void) sprintf(out_val, "%s %s %s.",
                       dstring,
                       is_a_vowel( c_list[j].name[0] ) ? "an" : "a",
                       c_list[j].name); /* No "pause" prompt in hist. window */
      historywindow_prt(out_val);    /* JCW Add text into the history window */
#endif
      if(recallwindowopen)
...
#if defined(ARC_RISCOS2) || defined(MAC)
      if(recallwindowopen) /* JCW - Automagically show recall info in window */
        {
          query = roff_recall(j, 0);
          query = inkey();
        }
      else
        {
...
#ifdef ARC_RISCOS2
      sprintf(out_val, "%s %s", dstring, tmp_str); /* don't want "pause" text*/
      historywindow_prt(out_val);    /* JCW Add text into the history window */
#endif
      ...
      if ((gl_rock || out_val[0]) && c_ptr->fval >= MIN_CLOSED_SPACE)
        {
          ...
          if (string)
            {
#ifdef ARC_RISCOS2
      sprintf(out_val, "%s %s", dstring, string); /* don't want "pause" text */
      historywindow_prt(out_val);    /* JCW Add text into the history window */
#endif
...
#ifdef COLOUR_GFX
          colour = object_list[throw_obj.index].colour;
#endif
          flag = FALSE;
          y = char_row;
          x = char_col;
