...
#ifdef COLOUR_GFX
          printattr((char)c_list[m_ptr->mptr].cchar,
                    (char)c_list[m_ptr->mptr].colour,
                    (int)m_ptr->fy,
                    (int)m_ptr->fx);
#else
...
char *bolt_typ;
{
  ...
  switch(typ)
    {
      case GF_MAGIC_MISSILE: colour = Colour_Magic_Missile; break;
      case GF_LIGHTNING:     colour = Colour_Lightning;     break;
      case GF_POISON_GAS:    colour = Colour_Poison_Gas;    break;
      case GF_ACID:          colour = Colour_Acid;          break;
      case GF_FROST:         colour = Colour_Frost;         break;
      case GF_FIRE:          colour = Colour_Fire;          break;
      case GF_HOLY_ORB:      colour = Colour_Holy_Orb;      break;
    }
#endif
  flag = FALSE;
      ...
      if ((dist > OBJ_BOLT_RANGE) || c_ptr->fval >= MIN_CLOSED_SPACE)
      ...
      else
        {
          if (c_ptr->cptr > 1)
            ...
            {
#ifdef COLOUR_GFX
              printattr('*', colour, y, x); /* show the bolt */
#else
              print('*', y, x); /* show the bolt */
#endif
              put_qio();
            }
        }
      ...
      if (dist > OBJ_BOLT_RANGE)
      ...
      else
        {
          ...
          if ((c_ptr->fval >= MIN_CLOSED_SPACE) || (c_ptr->cptr > 1))
            ...
            {
#ifdef COLOUR_GFX
              printattr('*', colour, y, x);      /* show bolt */
#else
              print('*', y, x);      /* show bolt */
#endif        
              put_qio();
