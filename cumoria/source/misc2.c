...
#ifdef MAC
extern int optionwindowopen; /* from macFrontEnd.c */
extern void Select_OptionWindow(void);
#endif
  ...
  for (max = 0; options[max].o_prompt != 0; max++)
    {
      ...
      (void) sprintf(string, "%-38s:", options[max].o_prompt);
      prt(string, max+1, 0);
      setattr(Colour_Title);
      (void) sprintf(string, "%s", (*options[max].o_var ? " yes " : " no  "));
      prt(string, max+1, 40);
#else
  ...
  for(;;)
    {
      ...
      (void) sprintf(string, "%s", (*options[i].o_var ? ">yes<" : ">no <"));
      prt(string, i+1, 40);
#else
      put_buffer(">", i+1, 40);
      put_buffer("<", i+1, 44);
