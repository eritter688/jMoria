...
#include <time.h>
#endif
#if !defined(GEMDOS) && !defined(MAC) && !defined(AMIGA) && !defined(ARC_RISCOS2)
#ifndef VMS
#include <sys/types.h>
#else
#include <types.h>
...
#if !defined(ATARIST_MWC) && !defined(MAC) && !defined(VMS) && !defined(AMIGA) && !defined(ARC_RISCOS2)
long time();
#endif
#ifndef ARC_RISCOS2
struct tm *localtime();
...
#ifdef COLOUR_GFX
extern char potion_colour[];
extern char mushroom_colour[];
extern char wood_colour[];
extern char metal_colour[];
extern char rock_colour[];
extern char amulet_colour[];
#endif
...
#ifdef COLOUR_GFX
/* Returns colour for given row, column                 -EMP-   */
unsigned char loc_colour(y, x)
int y, x;
{
  register cave_type *cave_ptr;
  register struct flags *f_ptr;
  ...
  if ((cave_ptr->cptr == 1) && (!find_flag || find_prself))
    ...
    return ((unsigned char) c_list[m_list[cave_ptr->cptr].mptr].colour);
  else if (!cave_ptr->pl && !cave_ptr->tl && !cave_ptr->fm)
    return Colour_White;
  else if ((cave_ptr->tptr != 0)
           && (t_list[cave_ptr->tptr].tval != TV_INVIS_TRAP))
    switch(t_list[cave_ptr->tptr].tval)
        {
        case TV_AMULET  : return amulet_colour[t_list[cave_ptr->tptr].subval &
                                          (ITEM_SINGLE_STACK_MIN - 1)];
        case TV_RING    : return rock_colour[t_list[cave_ptr->tptr].subval &
                                          (ITEM_SINGLE_STACK_MIN - 1)];
        case TV_STAFF   : return wood_colour[t_list[cave_ptr->tptr].subval &
                                          (ITEM_SINGLE_STACK_MIN - 1)];
        case TV_WAND    : return metal_colour[t_list[cave_ptr->tptr].subval &
                                          (ITEM_SINGLE_STACK_MIN - 1)];
        case TV_POTION1 :
        case TV_POTION2 : return potion_colour[t_list[cave_ptr->tptr].subval &
                                          (ITEM_SINGLE_STACK_MIN - 1)];
        case TV_FOOD    : if ((t_list[cave_ptr->tptr].subval &
                             (ITEM_SINGLE_STACK_MIN - 1)) < MAX_MUSH)
                          return mushroom_colour[t_list[cave_ptr->tptr].subval &
                                          (ITEM_SINGLE_STACK_MIN - 1)];
        default         : return ((unsigned char) \
                          object_list[t_list[cave_ptr->tptr].index].colour);
        }
  ...
  else if(cave_ptr->fval == MAGMA_WALL)
    {
      return Colour_Magma;
    }
  else if(cave_ptr->fval == QUARTZ_WALL)
    {
      return Colour_Quartz;
    }
      ...
      for (j = panel_col_min; j <= panel_col_max; j++)  /* Left to right */
        {
          ...
          if (tmp_char != ' ')
#ifdef COLOUR_GFX
            printattr(tmp_char, loc_colour(i, j), i, j);
#else
            print(tmp_char, i, j);
#endif
        }
