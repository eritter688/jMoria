...
#if defined(ARC_RISCOS2) || defined(MAC)
          if (recallwindowopen) /* show new info on what was killed */
            (void) roff_recall(m_ptr->mptr, 0);
#endif
        }
...
int y, x;
{
  ...
  if (!m_list[crptr].ml)
    ...
    {
#if defined(ARC_RISCOS2) || defined(MAC)
      if (recallwindowopen)     /* info on first hit of a series on one mon */
        (void) roff_recall(monptr, 0);
#endif
