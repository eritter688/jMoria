...
#ifdef MAC
#include "mac.h"
extern int	confirm_save(int);
extern int	savefileset;
#endif
...
#if !defined(ATARIST_TC) && !defined(ARC_RISCOS2)
#ifndef VMS
#include <pwd.h>
#else
#include <file.h>
...
#ifdef MAC		/* THINK_C seems to want functions declared */
void goback(void);
#endif
...
{
  register char *tmp;
#if defined(MAC) || defined(ARC_RISCOS2)
  time_t clockvar;
#else
  long clockvar;
#endif
...
#if !defined(AMIGA) && !defined(MAC) && !defined(ATARIST_TC) && !defined(ARC_RISCOS2)
...
#if defined(MSDOS) || defined(VMS) || defined(AMIGA) || defined(MAC) || defined(ARC_RISCOS2)
#if defined(MAC) || defined(MSDOS) || defined(ARC_RISCOS2)
...
static void print_tomb()
{
 ...
 retry:
  ...
  put_buffer ("('Y' - Yes / 'N' - No / 'F' - Save description in a file)", 23, 0);
  put_buffer ("Display character description [Y/N/F]?", 22, 0);
  do
    {
      func = inkey();
      switch (func)
...
static void highscores()
{
  ...
  int8u version_maj, version_min, patch_level;
  long curpos;
#if defined(VMS) || defined(MSDOS) || defined(AMIGA) || defined(MAC) || defined(ARC_RISCOS2)
  char string[100];
#endif
  ...
  if (turn >= 0)
    {
      if (total_winner)
        kingly();
      print_tomb();
      py.misc.chp = -1; /* This hack to tell scorefile the char is dead... */
    }
  if (character_generated && !character_saved)
#ifdef MAC
	if ((savefileset) || (confirm_save(FALSE) == saveYes))
#endif
...
#if defined(MAC) || defined(ARC_RISCOS2)	 /* Undo what has been done */
  goback();	/* Long jump back into the Mac wrapper, in lieu of exit () */
#else
  exit (0);
#endif
}
