...
#ifdef COLOUR_GFX
          (void) sprintf(out_val[i], "%s", tmp_val);         /* no "a)" etc. */
          l = strlen(out_val[i]) + 5;      /* but still reserve enough space */
#else
...
#ifdef COLOUR_GFX
          char zottemp[10];
          extern char itemcolour(inven_type *, int);

          setattr(Colour_Field);
          if (col == 0)
            sprintf(zottemp, "%c) ", 'a'+i); /* Don't need 1st 2 spaces */
          else
            sprintf(zottemp, "  %c) ", 'a'+i);

          prt(zottemp, current_line, col);

          setattr(itemcolour(&inventory[i], FALSE));
          if (col == 0)
            prt(out_val[i], current_line, col + 3);
          else
            prt(out_val[i], current_line, col + 5);
...
#ifdef COLOUR_GFX
  vtype out_val1[INVEN_ARRAY_SIZE-INVEN_WIELD];
#endif
  register inven_type *i_ptr;
...
#ifdef COLOUR_GFX
          (void) sprintf(out_val1[line], "  %c) %-14s:", line+ 'a', prt1);
          (void) sprintf(out_val[line], " %s", prt2);
          l = strlen(out_val[line]) + strlen(out_val1[line]);
#else
      ...
      if (i_ptr->tval != TV_NOTHING)
        {
          ...
          prt(zottemp, line+1, col);
          if (col == 0)
            prt(&out_val1[line][2], line+1, col);
          else
            prt(out_val1[line], line+1, col);

          setattr(itemcolour(&inventory[i], FALSE));
          if (col == 0)
            prt(out_val[line], line+1, col+18);
          else
            prt(out_val[line], line+1, col+20);
          setattr(Colour_Default);
#else
...
#ifdef ANSIPROTO
static char map_roguedir(register char comval)
#else
...
#ifdef COLOUR_GFX
            printattr(loc_symbol(i, j), loc_colour(i, j), i, j);
#else
...
#ifdef COLOUR_GFX
    printattr(loc_symbol(y, x), loc_colour(y, x), y, x);
#else
...
#ifdef COLOUR_GFX
    printattr(loc_symbol(y1, x1), loc_colour(y1, x1), y1, x1);
#else
